	...
	for (int i = 0; i < size; ++i) {
		for (int j = 0; j < size; ++j) {
			if (!settled){
				...
				if (i == 6) {
					colour = 'B';
					theBoard[i][j].setPiece('P', colour, theBoard);	//const char?
				} else if (i == 1) {
					colour = 'W';
...
/*void Board::computerMove(int r1, char c1, int rand) {
	int row = r1 - 1;
	int col = c1 - 'a';
	std::pair<int, int> pos = theBoard[row][col].getMoveCoord(rand);
	move(r1, c1, pos.first, pos.second);
}*/
...
int Board::fromSize(char co, int level) {
    ...
    for(int i = 0; i < size; ++i) {
        for(int j = 0; j < size; ++j) {
            if (theBoard[i][j].getColour() ==  co && !(theBoard[i][j].isEmpty())
                && (theBoard[i][j].sizeMove() != 0)) {
                cout << "can be moved: " << i << "," << j << endl;
                cout << "size of attack is " << theBoard[i][j].sizeAttack() << endl;
                choices[n][0] = i;
                choices[n][1] = j;
    ...
    cout << "countA is " << countA << "countC is " << countC << endl;
    if (level == 3) {
        if (countA > 0) {
            for (int s = 0; s < countA; ++s) {
                ...
                for (int num = 0; num < sm; ++num) {
                    ...
                    cout << "A:" << "r, c" << r << "," << c << "i, j" << i << "," << j << endl;
                    cout << "num is " << num << "sm is " << sm << endl;
                    cout << "countAC is " << countAC << endl;
...
int Board::toSize(int rand, int level) {
    ...
    //choices[0][2] = choices[rand][2];
    //choices[0][3] = choices[rand][3];
    //choices[0][4] = choices[rand][4];
    if (level == 2) {
        //cout << "attack: " << choices[0][3] << ", check: " << choices[0][4] << endl;
    ...
    //cout << "rand is " << rand << ", toSize: " << choices[0][2] << endl;
...
void Board::computerMove(int rand, int level) {
    int i = choices[0][0];
    int j = choices[0][1];
    cout << "computerMove: i, j" << i << "," << j << endl;
    if (level == 1) {
        //cout << "test: r is" << theBoard[i][j].getMoveCoord(0).first << " c is " << theBoard[i][j].getMoveCoord(0).second << endl;
        int r = theBoard[i][j].getMoveCoord(rand).first;
        int c = theBoard[i][j].getMoveCoord(rand).second;
        //cout << "rand is " << rand << ", from " << i << "," << j << " to " << r << "," << c << endl;
...
void Board::move(int r1, char c1, int r2, char c2) {
	// Game() should make sure the inputs are valid
	int row = r1 - 1;
	int col = c1 - 'a';
	int row2 = r2 - 1;
	int col2 = c2 - 'a';
